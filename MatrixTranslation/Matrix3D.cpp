...
Matrix3D MakeTranslation(Vector2D v)
{
	return Matrix3D(1.0f, 0.0f, v.x,
		0.0f, 1.0f, v.y,
		0.0f, 0.0f, 1.0f);
}

Vector3D Translate(Vector2D v, Vector2D r)
{
	Vector3D originalVector = Vector3D(v, 1.0f);
	Matrix3D translateMatrix = MakeTranslation(r);

	return translateMatrix * originalVector;
}
